...
function listPaddockManagersByName() {
  // CODE HERE
  const names = paddockManagers.map((paddockManagers)=> paddockManagers.name);
  return names.sort();
};
...
{
    const paddocksWithTotalArea = Object.assign([], paddockType); // Corregir el uso de Object.assign
  
    let areaID1 = 0, areaID2 = 0, areaID3 = 0, areaID4 = 0;
  
    for (let i = 0; i < paddocks.length; i++) { // Corregir condición en el bucle for
      switch (paddocks[i].paddockTypeId) {
        case 1:
          areaID1 += paddocks[i].area;
          break;
        case 2:
          areaID2 += paddocks[i].area;
          break;
        case 3:
          areaID3 += paddocks[i].area;
          break;
        case 4:
          areaID4 += paddocks[i].area;
          break;
      }
    }
  
    const totalAreaPerType = [areaID1, areaID2, areaID3, areaID4];
    
    paddocksWithTotalArea.forEach((object, index) => {
      object.area = totalAreaPerType[index]; // Asignar el área total a cada tipo de cuartel
    });
  
    // Ordenar la lista de cuarteles según el área total de cada tipo
    paddocksWithTotalArea.sort((a, b) => b.area - a.area);
